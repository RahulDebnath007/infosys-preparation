...
For each book, check shelves from left to right.

Place the book on the first unused shelf whose capacity is greater than or equal to the book's thickness.

Once a shelf is used, it cannot be used again.

If no suitable shelf is available, the book remains unplaced.
...
Scan all shelves from left to right.
...
If no suitable shelf is found, increment unplaced.
...
Process elements in given order
